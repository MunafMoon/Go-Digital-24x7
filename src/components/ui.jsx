...
export function Reveal({ children, className = '' }) {
  ...
  return <motion.div ref={ref} className={className} initial={{ opacity: 0, y: 28 }} animate={show ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.55 }}>{children}</motion.div>;
}

export function SectionTitle({ eyebrow, title, subtitle, dark = false }) {
  return <Reveal className="mx-auto mb-12 max-w-3xl text-center"><p className="mb-3 text-sm font-extrabold uppercase tracking-[.22em] text-[#6C3BFF]">{eyebrow}</p><h2 className={`text-3xl font-extrabold md:text-5xl ${dark ? 'text-white' : 'text-[#0B1020]'}`}>{title}</h2>{subtitle && <p className={`mt-5 text-lg ${dark ? 'text-white/70' : 'text-slate-600'}`}>{subtitle}</p>}</Reveal>;
}

export function GradientButton({ children, to = '#audit', ghost = false }) {
  const href = to.startsWith('/') ? `#${to}` : to;
  return <a className={`btn ${ghost ? 'btn-ghost' : 'btn-primary'}`} href={href}>{children}</a>;
}
